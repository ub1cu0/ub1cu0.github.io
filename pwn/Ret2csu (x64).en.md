...
This one is like [callme](https://ub1cu0.gitbook.io/pwn-writeups/ropemporium/3.-callme-x86_64) but without having to jump into 3 functions. I only need one jump to the `ret2win` function. Let me try it and see what wall I hit this time:
...
As I can see, the `pwnme` function has a buffer overflow with plenty of room, which gives me space for a long rop chain. Now let me look at the `ret2win` function:
...
Just like in [callme](https://ub1cu0.gitbook.io/pwn-writeups/ropemporium/3.-callme-x86_64), this challenge wants 3 arguments before it prints the flag (`RDI`, `RSI` and `RDX`):

Let me look for gadgets to load those registers with the values I need:
...
There is a problem, there is no `pop rdx` anywhere. That is the new difficulty of this challenge. To get around it I have to use, just like the name of the challenge suggests, the `__libc_csu_init` function. It is generated automatically when a program is compiled and it always brings a couple of pretty useful gadgets: one that does several `pop` in a row and another one that does `mov rdx, r15; mov rsi, r14; mov edi, r13d; call [r12+rbx*8]`. With this I can set `rdx` even though there is no plain `pop rdx`, and pass every argument I need. I will use this image as a reference:
...
As I can see, I can call gadget 1 first to set several registers, and gadget 2 then moves them into the registers I needed. I have to keep in mind that gadget 2 has a call, so I need this:

```c
400689:	41 ff 14 dc          	call   QWORD PTR [r12+rbx*8]
```

to land on a function that returns and does not touch any important register. Another thing to keep in mind is that in gadget 1 I have to set `rbx` to the value of `rbp - 1`, because the gadget has these lines:
...
If that does not hold it jumps into another function, and I do not want that. I also have to keep in mind that on the second pass the pops run again, so I have to pop something 7 times, 6 times for the `pop` and 1 for the `add`. Now that I know this I can put the exploit together:
